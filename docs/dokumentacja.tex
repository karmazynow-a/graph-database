...
\subsection{Model}
Aplikacja zawiera jeden model danych - model przystanku, który jest reprezentowany przez węzeł w bazie. Identyfikowany jest on za pomocą nazwy (oprócz standardowego id, nadawanego przez bazę Neo4j). Operacje na węzłach są wykonywane przez zapytania języka Cypher, generowane na podstawie danych przesłanych od widoku z formaularzy. Przykładowe zapytania:
\begin{itemize}
